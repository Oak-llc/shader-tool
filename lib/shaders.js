...
import { getAuth, getMongo, PersistenceUnavailableError } from './auth.js';
...
async function requireSession(req, res, next) {
  try {
    ...
    handlePersistenceError(err, res, next);
  }
}
...
async function getCollections() {
  ...
  return {
    shaders: mongo.collection('shaders'),
    likes: mongo.collection('likes'),
  };
}

function handlePersistenceError(err, res, next) {
...
shaderRoutes.post('/shaders', requireSession, asyncHandler(async (req, res) => {
  const { shaders } = await getCollections();
...
// Owner-only delete.
shaderRoutes.delete('/shaders/:id', requireSession, asyncHandler(async (req, res) => {
  const { shaders, likes } = await getCollections();
...
shaderRoutes.get('/shaders/:id', asyncHandler(async (req, res) => {
  const { shaders } = await getCollections();
  const id = toId(req.params.id);
  if (!id) return res.status(400).json({ error: 'invalid id' });
  const doc = await shaders.findOne({ _id: id });
...
shaderRoutes.get('/users/:handle/shaders', asyncHandler(async (req, res) => {
  const { shaders } = await getCollections();
...
shaderRoutes.get('/shaders/feed/recent', asyncHandler(async (req, res) => {
  const { shaders } = await getCollections();
  const docs = await shaders
    .find({ public: true })
    .sort({ createdAt: -1 })
...
shaderRoutes.post('/shaders/:id/like', requireSession, asyncHandler(async (req, res) => {
  const { shaders, likes } = await getCollections();
  const id = toId(req.params.id);
  if (!id) return res.status(400).json({ error: 'invalid id' });
